...
def main(
    ...
    with StateManager() as manager:
        ...
        overlay_socket = None
        if args.keyboard.lower() == "local_full":
            from PiFinder.keyboard_local_full import OverlaySocket
                    ...
                    for event in pygame.event.get():
                        if event.type == pygame.KEYDOWN:
                            ...
                            if ctrl_held and event.key in pygame_ctrl_key_map:
                                key_code = pygame_ctrl_key_map[event.key]
                            elif event.key in pygame_key_map:
                                key_code = pygame_key_map[event.key]
                            if key_code is not None:
                                keyboard_queue.put(key_code)
                                if overlay_socket:
                                    overlay_socket.send(key_code)
...
if __name__ == "__main__":
    ...
    elif args.keyboard.lower() == "local_full":
        from PiFinder import keyboard_local_full as keyboard  # type: ignore[no-redef]

        rlogger.info("using local keyboard with overlay socket (pygame main loop + socket)")
