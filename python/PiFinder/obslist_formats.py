...
SUPPORTED_EXTENSIONS = (
    ".skylist", ".csv", ".sol", ".hct", ".lst", ".txt", ".mtf", ".pifinder",
)
...
def write_csv(obs_list: ObsList) -> str:
    ...
    for entry in obs_list.entries:
        mag_str = f"{entry.mag.filter_mag:.1f}" if entry.mag.filter_mag != MagnitudeObject.UNKNOWN_MAG else ""
        writer.writerow(
            [
                entry.name,
        ...
        "objects": [
            {
                ...
                "magnitude": f"{entry.mag.filter_mag:.2f}" if entry.mag.filter_mag != MagnitudeObject.UNKNOWN_MAG else "",
            }
            for entry in obs_list.entries
        ],
    }
...
def write_autostar(obs_list: ObsList) -> str:
    ...
    for entry in obs_list.entries:
        ...
        mag_str = f" mag {entry.mag:.1f}" if entry.mag.filter_mag != MagnitudeObject.UNKNOWN_MAG else ""
...
def write_argo(obs_list: ObsList) -> str:
    ...
    for entry in obs_list.entries:
        ...
        mag_str = f"{entry.mag.filter_mag:.1f}" if entry.mag.filter_mag != MagnitudeObject.UNKNOWN_MAG else "ANY"
