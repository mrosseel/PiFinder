...
def main():
    parser = argparse.ArgumentParser(description="Fetch survey images for PiFinder objects")
    parser.add_argument("--force", action="store_true", help="Re-fetch even if image exists")
    ...
    if args.force:
        ...
        for ra, dec, name in objects:
            poss_missing = do_poss and not os.path.exists(resolve_image_path(name, "POSS"))
            sdss_missing = do_sdss and not os.path.exists(resolve_image_path(name, "SDSS"))
