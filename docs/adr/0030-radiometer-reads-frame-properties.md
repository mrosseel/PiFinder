...
tuning tuple, which is exactly what ADR 0023's one-count sentinel exists to
distinguish. Taking it anyway raises that unit's own mean absolute error over
...
Nothing changes here. ADR 0023 and ADR 0028 already decide the pedestal
correctly.
...
- Published live values do not move. Over all 66 referenced archive sweeps, mr2,
  ...
  publication counts are unchanged.
- Archive mean absolute error falls from 0.251 to 0.202 mag. The unit reporting
  a folded gain falls from 0.762 to 0.378; on the airglow-floor path the same
  two corrections take it from 0.506 to 0.117.
