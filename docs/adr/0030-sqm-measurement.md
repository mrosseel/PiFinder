...
This is the single decision record for the SQM estimator. It replaces seven
...
2. a valid same-frame **optical black** measurement (§4.1);
3. a **leased tracked black level** (§4.2);
4. an optional user calibration's bias offset;
5. the profile `bias_offset`.

### 4.1 Per-frame optical black on IMX290/IMX462

On these sensors the pedestal is the sensor's own shielded optical-black rows,
measured on **every frame**, rather than a static offset or a nightly-fitted
dark-current model.
...
1920×1080 image matrix. A libcamera cam-helper unpacks the shielded pixels,
trims the outer 5% each side, averages the central 90%, and publishes the result
through `controls::SensorBlackLevels` on the 16-bit scale, tagged
...
sample as `optical_black_pedestal`.

A valid marked OB value is the *complete* per-frame pedestal: it already
contains the bias and the frame's accumulated dark signal. No dark-current rate
is applied on top of it, and no nightly rate is fitted or persisted for these
sensors. Same-frame OB also makes scheduled pedestal-probe frames unnecessary,
which is why that earlier proposal was never implemented.

### 4.2 The tracked black level supersedes any stored bias
...
The profile constant is right and the reported value is the static tuning tuple,
which is exactly what §4.1's sentinel exists to reject. Taking it anyway raises
...
The same table discharges §4.2's dark-site obligation: on the two clear
...
- *Trusting any reported black level without a measured-value marker.* §4.3.
- *A stored bias measured once.* §4.2. It cannot represent a quantity that
...
- *Scheduled pedestal-probe frames.* Unnecessary once same-frame OB exists.
...
| 0023 | per-frame optical black as the IMX290/462 pedestal | §4.1 |
...
| 0028 | the tracked black level supersedes any stored bias | §4.2 |
| 0030 | frame extent and reported digital gain are read, not assumed | §3, §4.3 |
