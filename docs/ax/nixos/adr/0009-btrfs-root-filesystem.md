...
- **Kernel-written files: a fault my first test missed.** The first tests used images made by `mkfs.btrfs --rootdir`. On a device, the kernel rewrites `extlinux.conf` on every upgrade, rollback and camera switch. A kernel that compresses the whole block (7.2.7 does, 6.18.53 does not; PiFinder runs 6.12) stores that ~500-byte file as a compressed inline extent, and U-Boot 2026.04 cannot read it: "zstd_decompress: failed to decompress: 70", `sysboot` fails, and the device would not boot after its first upgrade. Reproduced in QEMU after kernel 7.2.7 rewrote `extlinux.conf` (`nixos/tests/btrfs-spike/kernelwrite.nix`). U-Boot fixed it upstream in `6f0719e4c4` (on `next`, 22 August 2026, not in 2026.04). With that commit as a local patch in `ubootSD`, the same file reads and `sysboot` reaches "Starting kernel". Every later spike step must also test files the kernel wrote, not only files from the image build.
...
- **U-Boot patches.** `ubootSD` carries two local btrfs patches: the backport of `6f0719e4c4` (kernel-written inline extents) and the copy fallback. A U-Boot update must check that both still apply, and drop the backport once the release has it.
