# Copy of nixpkgs nixos/lib/make-btrfs-fs.nix (26.05) with transparent
# compression and a choice of data and metadata profiles (NixOS ADR 0009).
#
...
# that a script resizes the filesystem at boot time.
{
  ...
  # mkfs.btrfs --compress: files are written compressed.
  compression ? "zstd:3",
  # "single" or "dup" (two copies on the one device).
  dataProfile ? "single",
  metadataProfile ? "dup",
  btrfs-progs,
  libfaketime,
...
pkgs.stdenv.mkDerivation {
  ...
  buildCommand = ''
    ...
    faketime -f "1970-01-01 00:00:01" fakeroot mkfs.btrfs -L ${volumeLabel} -U ${uuid} \
      -d ${dataProfile} -m ${metadataProfile} --compress ${compression} \
